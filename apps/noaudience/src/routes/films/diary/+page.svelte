...
<div class="max-w-4xl">
  <h1 class="text-3xl font-bold text-white mb-8">Diary</h1>

  {#each groupedLogs() as group}
    <section class="mb-8">
      <h2 class="text-sm font-semibold text-[#99AABB] uppercase tracking-wider mb-3">{group.label}</h2>
      <div class="bg-[#1B2028] rounded-[8px] border border-white/[0.06] overflow-hidden">
        {#each group.entries as entry, i}
          <a
            ...
            class="flex items-center gap-4 px-4 py-3 hover:bg-[#2C3440] transition-colors {i < group.entries.length - 1 ? 'border-b border-white/[0.04]' : ''}"
          >
            <!-- Date -->
            <div class="w-10 text-center flex-shrink-0">
              ...
              <div class="text-[10px] text-[#99AABB] uppercase">
  ...
  {#if logs.length === 0}
    <div class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-[#99AABB]">No diary entries yet.</p>
      <p class="text-[#667788] text-sm mt-1">Log a film to get started.</p>
    </div>
  {/if}
</div>
