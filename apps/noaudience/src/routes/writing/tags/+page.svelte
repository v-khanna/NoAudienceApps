<script lang="ts">
  ...
  import { getAllTags } from '$lib/writing/db';
...
<div style="display: flex; align-items: center; gap: 16px; margin-bottom: 36px;">
  ...
  <h1 style="font-size: 28px; font-weight: 700; color: var(--text-primary); margin: 0;">Tags</h1>
  <div style="flex: 1;"></div>
  <span style="font-size: 13px; color: var(--text-tertiary);">{tags.length} tags</span>
</div>
...
{:else if tags.length === 0}
  <div style="padding: 48px 0; text-align: center;">
    <p style="font-size: 15px; color: var(--text-tertiary);">No tags yet</p>
  </div>
{:else}
  <div>
    {#each tags as { tag, count }, i}
      <div
        ...
        "
      >
        <span style="font-size: 15px; color: var(--text-primary); flex: 1;">#{tag}</span>
